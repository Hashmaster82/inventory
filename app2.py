...
class InventoryApp:
    ...
    def open_transfer_dialog(self, equipment_item):
        ...
        info_frame = ttk.LabelFrame(dialog, text="Информация об оборудовании", padding=10)
        ...
        for i, (label_text, value) in enumerate(info_labels):
            ttk.Label(info_frame, text=label_text, font=self.default_font).grid(row=i, column=0, sticky='w', padx=(0, 10))
            ttk.Label(info_frame, text=value, font=self.default_font, wraplength=400).grid(row=i, column=1, sticky='w')

        transfer_frame = ttk.LabelFrame(dialog, text="Новое закрепление", padding=10)
        transfer_frame.pack(fill='x', padx=10, pady=10)

        ttk.Label(transfer_frame, text="Сотрудник:", font=self.default_font).pack(anchor='w')
        new_employee_var = tk.StringVar()
        employee_combo = ttk.Combobox(transfer_frame, textvariable=new_employee_var,
                                      values=sorted(self.employees_list), state='readonly', font=self.default_font)
        ...
        ttk.Label(transfer_frame, text="Дата передачи:", font=self.default_font).pack(anchor='w', pady=(10, 0))
        date_entry = ttk.Entry(transfer_frame, textvariable=date_var, font=self.default_font)
        ...
        ttk.Button(btn_frame, text="Подтвердить", command=confirm_transfer, style='Small.TButton').pack(side='left', padx=5)
        ttk.Button(btn_frame, text="Отмена", command=dialog.destroy, style='Small.TButton').pack(side='left', padx=5)

        dialog.wait_window()

    # =============== ОСТАЛЬНЫЕ МЕТОДЫ БЕЗ ИЗМЕНЕНИЙ ===============
    # (все остальные методы из оригинального кода остаются без изменений)
